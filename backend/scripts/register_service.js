...
/**
 * Git Bash on Windows silently converts POSIX-style arguments like /temp1 into
 * absolute Windows paths (e.g. C:/Program Files/Git/temp1). Detect and reverse
 * that conversion so stored paths are always clean POSIX-style strings.
 *
 * Examples:
 *   "C:/Program Files/Git/temp1"    → "/temp1"
 *   "C:/Program Files/Git/temp1/*"  → "/temp1/*"
 *   "/temp1"                        → "/temp1"  (unchanged)
 */
function sanitizePosixArg(value) {
...
async function registerGatewayService() {
  ...
  console.log(
    "This script will register a new backend service and its routing configuration into the Gateway.\n",
  );
  ...
  try {
    ...
    if (process.argv.length >= 7) {
      ...
      routePath = sanitizePosixArg(process.argv[4]);
      stripPrefix = sanitizePosixArg(process.argv[5]);
      requiresAuth = process.argv[6].trim().toLowerCase() === "y";
      console.log(
        `Using provided arguments: ${name}, ${baseUrl}, ${routePath}, ${stripPrefix}, ${process.argv[6]}`,
      );
    } else {
      name = await askQuestion(
        'Enter Service Name (e.g., "Auth Service", "Temp Server 1"): ',
      );
      if (!name.trim()) throw new Error("Service Name is required.");

      baseUrl = await askQuestion(
        'Enter Base URL (e.g., "http://localhost:3001", "http://temp:3000"): ',
      );
      if (!baseUrl.trim()) throw new Error("Base URL is required.");

      routePath = await askQuestion(
        'Enter Route Path match (e.g., "/auth/*", "/temp1/*"): ',
      );
      if (!routePath.trim()) throw new Error("Route Path is required.");

      stripPrefix = await askQuestion(
        'Enter Route Prefix to strip before forwarding (e.g., "/auth", "/temp1" or leave empty to strip nothing): ',
      );

      const requiresAuthInput = await askQuestion(
        "Does this route require Authentication? (y/N): ",
      );
    ...
    if (!route) {
      ...
      console.log(
        `ℹ️  Route already exists: ${routePath}. Updating configuration...`,
      );
    ...
    console.log(
      `Target Backend: URL: ${baseUrl}${stripPrefix.trim() ? `, Strip Prefix: ${stripPrefix}` : ""}`,
    );
  } catch (error) {
